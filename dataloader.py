...
class NuscData(torch.utils.data.Dataset):
    ...
    def token_to_vector(self, vehicle_anns):
        ...
        for token in vehicle_anns:
            ...
            # ignore vehicles that don't have a velocity
...
def collate_fn(output_list):
    ...
    batched_map = torch.stack([torch.tensor(output.map_mask) for output in output_list]).float()

    # concatenate the vehicles for each sample
    # have an additional tensor that indicates which batch each vehicle belong to
    batched_vehicles = torch.cat([torch.tensor(output.vehicles) for output in output_list], 0).float()
